...
def remove_files(root, zf2):
    def _remove(list_files):
        for k in list_files:
            new_path = os.path.join(root, k)
    ...
    tmp = [k.filename for k in zf2.filelist if k.is_dir()]
...
def unzip(zip_file_path, root=None):
    """Unzip the files."""
    root_temp = False
    if root is None:
        root_temp = True
        root = os.environ.get("DATA_PATH_DIR", os.path.expanduser("./"))
    folders = []

    with zipfile.ZipFile(zip_file_path) as zf:
        list_files = zf.namelist()
        print("Removing Files")
        remove_files(root, zf)
...
def download(url, path=None, overwrite=False, sha1_hash=None):
    ...
    """
    path = os.environ.get("DATA_PATH_DIR", None)
    if path is None:
        fname = url.split("/")[-1]
    else:
